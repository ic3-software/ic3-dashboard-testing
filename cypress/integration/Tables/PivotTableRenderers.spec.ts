...
function getTableCell(widgetId: string, rowIdx: number, colIdx: number) {
    return cy.getWidget(widgetId).find('.ic3-pt-rows div[data-r="' + rowIdx + '"][data-c="' + (colIdx - 1) + '"]')
}

// Same code as TableRenderers
describe("Tables/Pivot Table Renderers", () => {

    beforeEach(() => {
        cy.login();
        cy.openViewerTestReport("Tables/Pivot Table Renderers");
